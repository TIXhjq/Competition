...
def cal_windows_fea(df:pd.DataFrame, cal_col:str, stat_dim:list, data_type:str) -> pd.DataFrame:
    ...
    # 滑窗特征
    ## 均值
    feature_data = train_sales_data.groupby(stat_dim)[cal_col].apply(lambda x: x.sum()).groupby(stat_dim[:-1]).rolling(3).sda
...
result.forecastVolum=result.forecastVolum*0.95
result.forecastVolum = result.forecastVolum.astype(int)

print((result.forecastVolum < 0 ).sum())
result[["id", "forecastVolum"]].to_csv("../submit/evaluation_public_201901017_lgb.csv", index=False)